...
const { PERMISSIONS, ROLES, hasPermission } = require('../backend/src/rbac/roles');
...
test('admin can issue refunds', () => {
  assert.equal(hasPermission('admin', PERMISSIONS.REFUND_ISSUE), true);
});
...
test('only admin has ADMIN_OPS permission', () => {
  assert.equal(hasPermission('admin', PERMISSIONS.ADMIN_OPS), true);
  assert.equal(hasPermission('analyst', PERMISSIONS.ADMIN_OPS), false);
  assert.equal(hasPermission('moderator', PERMISSIONS.ADMIN_OPS), false);
  assert.equal(hasPermission('finance', PERMISSIONS.ADMIN_OPS), false);
});

test('only admin has MANAGE_LAN_ALLOWLIST permission', () => {
  assert.equal(hasPermission('admin', PERMISSIONS.MANAGE_LAN_ALLOWLIST), true);
  assert.equal(hasPermission('analyst', PERMISSIONS.MANAGE_LAN_ALLOWLIST), false);
  assert.equal(hasPermission('moderator', PERMISSIONS.MANAGE_LAN_ALLOWLIST), false);
  assert.equal(hasPermission('finance', PERMISSIONS.MANAGE_LAN_ALLOWLIST), false);
});
